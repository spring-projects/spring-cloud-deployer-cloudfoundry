...
public class CloudFoundryAppDeployer implements AppDeployer {
	...
	private String buildpack(AppDeploymentRequest request) {
		return request.getDeploymentProperties().getOrDefault(CloudFoundryDeployerProperties.BUILDPACK_PROPERTY_KEY, properties.getBuildpack());
	}
	...
	private boolean useSpringApplicationJson(AppDeploymentRequest request) {
		return Boolean.valueOf(
				request.getDeploymentProperties().getOrDefault(CloudFoundryDeployerProperties.USE_SPRING_APPLICATION_JSON_KEY, valueOf(properties.isUseSpringApplicationJson())));
	}
